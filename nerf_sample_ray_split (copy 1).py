...
class RaySamplerSingleImage(object):
    def __init__(self, H, W, intrinsics=None, c2w=None,
        ...
        if img_path is not None:
            ...
            self.label_path = self.img_path[:-13] + '/cube_filt7/' + number
        else:
        ...
        if make_class_label:
            ...
            os.makedirs(self.img_path[:-13] + '/cube_filt7/', exist_ok=True)
        ...
        if resolution_level != self.resolution_level:
            ...
            if self.depth_path is not None:
                ...
                if self.train_box_only:
                    ...
                    self.depth_map = near + self.depth_map / 255. * (far - near)
    ...
    def depth_normalize(self,depth_map):
        ...
        depth_map_norm = depth_map / 10. # 30. in normal scene
    ...
    def get_classifier_label_torch_boxonly(self, N_front_sample, pretrain, select_inds=None, save=False, rank=0,
        ...
        if pretrain and save:
            ...
            Z = 11 # triangle filter 
            tri_fil = []
            for i in range(int(-np.floor(Z/2)), int(np.floor(Z/2))+1):
                    tri_fil.append((np.floor(Z/2)+1-np.absolute(i))/(np.floor(Z/2)+1))
                    torch.Tensor(tri_fil).unsqueeze(0)   
    ...
    def get_classifier_label_torch(self, N_front_sample, N_back_sample, pretrain, select_inds=None,save=False, rank=0, train_box_only=False):
        import time
        time_program = False
        if time_program:
            cur_time = time.time()
    ...
    def random_sample_classifier(self,N_rand, N_front_sample, N_back_sample,pretrain,rank):
        ...
        with torch.no_grad():
            ...
            if self.train_box_only:
                axis_filtered_depth_flat, fg_pts_flat, fg_z_vals_centre \
                         = self.get_classifier_label_torch_boxonly(N_front_sample, pretrain, select_inds=select_inds, rank=rank, train_box_only=self.train_box_only)
